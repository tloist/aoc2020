...
object Part2 {
  ...
  given PassportEvaluationPolicy with {
    ...
    def validate(passport: Passport): Boolean = {
      val combinedValidation = Kleisli.endoMonoidK[ValidationResult].algebra.combineAll(List(
        validateBirthYear, validateIssueYear, validateExpirationYear, validateHeight, validateHairColor, validateEyeColor, validatePassportId
      ).map(Kleisli.apply))
      combinedValidation.run(passport).isRight
    }
  }
